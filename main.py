...
def main():
    ...
    for i in range(0, len(db_h1n1), 100):
        ...
        if alignment:
            db_total_residues = sum(len(str(r.seq)) for r in db_h1n1)
            alignment["bit_score"] = round(calculate_bit_score(alignment["score"], BLASTN_PARAMS), 4)
            alignment["e_value"]   = round(calculate_e_value(alignment["score"], len(query), db_total_residues, BLASTN_PARAMS), 4)
